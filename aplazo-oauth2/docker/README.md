...
This document describes how to deploy the ORY Hydra service. The file
`docker.yml` is provided as a reference for configuration and environment
variables.
...
> **Note:** The admin port (4445) must remain private and only accessible via
> the Aplazo VPN.
...
Environment variables are defined in `.env.example`. The DevOps team must
provide at least:
...
The service health is checked using the following endpoint on the admin port
(4445):
...
services:
  hydra:
    ...
    healthcheck:
      test:
        ["CMD", "wget", "--spider", "-q", "http://127.0.0.1:4445/health/ready"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 10s
...
For any questions or further configuration, please refer to the official
